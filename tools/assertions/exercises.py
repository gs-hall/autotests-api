...
def assert_exercise(actual, expected):
    """
    Проверяет, что фактическая модель задания соответствует ожидаемой.
    :param actual: ExerciseSchema из ответа API
    :param expected: ExerciseSchema из запроса/создания
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
...
def assert_get_exercise_response(get_exercise_response, create_exercise_response):
    """
    Проверяет, что ответ на получение задания совпадает с ответом на его создание.
...
def assert_exercise_not_found_response(actual: InternalErrorResponseSchema):
    ...
    :param actual: Ответ API (InternalErrorResponseSchema)
